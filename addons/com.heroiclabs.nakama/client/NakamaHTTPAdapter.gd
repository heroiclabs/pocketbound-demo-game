...
func send_async(p_method : String, p_uri : String, p_headers : Dictionary, p_body : PackedByteArray):
	...
	for k in p_headers:
		if str(k).to_lower() == "content-length":
			has_content_length = true
		headers.append("%s: %s" % [k, p_headers[k]])

	# Some gateways reject POST/PUT/DELETE requests without an explicit Content-Length header.
	if (method == HTTPClient.METHOD_POST or method == HTTPClient.METHOD_PUT or method == HTTPClient.METHOD_DELETE) and not has_content_length:
		headers.append("Content-Length: %d" % p_body.size())
...
func _cancel_pending_requests() -> void:
	...
	var pending_ids := _pending.keys()
	for pending_id in pending_ids:
		if not _pending.has(pending_id):
			continue

		var req := _pending[pending_id] as AsyncRequest
		if req == null:
		...
		if is_instance_valid(req.request) and not req.request.is_queued_for_deletion():
			req.request.queue_free()
